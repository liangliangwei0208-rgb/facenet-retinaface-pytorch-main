...
### c、 数据集下载
**问：up主，XXXX数据集在哪里下载啊？
答：一般数据集的下载地址会放在README里面，基本上都有；没有的话可以在项目问题反馈中补充**。
...
答：建议按照推荐配置和配置教程安装。其它版本未逐一验证，可能出现问题，但一般只需要少量代码调整。**
...
**其它有很多稀奇古怪的问题，很多是版本问题，建议按照配套教程安装Keras和tensorflow。比如装的是tensorflow2，就不适合直接运行Keras-yolo这类依赖旧版本环境的代码。**
...
答：不同网络的LOSS不同，LOSS只是一个参考指标，用于查看网络是否收敛，而非评价网络好坏。部分代码没有对LOSS做归一化，所以LOSS值看起来可能偏高；重点是LOSS是否变小、预测是否有效果。**
...
**问：怎么进行多GPU训练？
答：pytorch的大多数代码可以直接使用gpu训练。keras多卡训练可以查阅对应框架文档，实现并不复杂，但不同环境需要自行测试。**
### d、灰度图问题
**问：能不能训练灰度图（预测灰度图）啊？
答：大多数代码会将灰度图转化成RGB进行训练和预测。如果遇到代码不能训练或者预测灰度图的情况，可以尝试在get_random_data里面将Image.open后的结果转换成RGB，预测时也这样处理。（仅供参考）**
...
答：基本上可以达到，已使用VOC数据做过测试；如果没有COCO训练条件，可以先用自己的数据集验证实际效果。**

**问：你有没有实现yolov4所有的tricks，和原版差距多少？
答：并没有实现全部的改进部分。YOLOV4使用的改进很多，很难完全实现与列出，这里只列出一些常见且有效的改进。论文中提到的SAM（注意力机制模块），原版源码也没有使用。还有其它很多tricks，不是所有tricks都有提升；至于和原版的比较，建议结合自己的数据集实际测试。**
...
答：建议看看yolov3和yolov4的区别，然后看看yolov4的论文，作为一个大型调参现场非常有参考意义，使用了很多tricks。建议多看经典模型，再拆解里面的亮点结构并尝试使用。**

### r、部署问题
目前没有完整的手机端部署验证记录，所以很多部署问题需要结合目标设备和框架文档排查……
...
**问：up，怎么修改模型啊，我想发个小论文！
答：建议看看目标检测中yolov4的论文，作为一个大型调参现场非常有参考意义，使用了很多tricks。建议多看经典模型，再拆解里面的亮点结构并尝试使用。常用的tricks如注意力机制等，可以按需尝试。**
...
答：这里有几点需要注意哈
1、这份学习路线仅供参考，不一定适用于所有人。
2、我实验室不做深度学习，所以我很多东西都是自学，自己摸索，正确与否我也不知道。
3、我个人觉得学习更靠自学**
学习路线可以从Python基础开始，再依次学习tensorflow、keras、pytorch入门，然后学习SSD、YOLO等目标检测模型，继续了解经典卷积网络。阅读代码时建议一行一行看，理解执行流程和特征层shape变化。这个过程需要持续投入时间，没有太多捷径。
