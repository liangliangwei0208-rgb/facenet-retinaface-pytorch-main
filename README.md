...
```text
https://github.com/liangliangwei0208-rgb/facenet-retinaface-pytorch-main
```

模型权重文件 `model_data/*.pth` 使用 Git LFS 管理。Gitee 镜像用于同步代码和 LFS 指针文件，受 Gitee LFS 支持限制，镜像仓库中的 `.pth` 可能不是完整权重文件。

如果从 Gitee 克隆项目后发现模型权重无法正常加载，请从 GitHub 主仓库拉取完整 LFS 权重，或手动替换 `model_data/` 下对应的 `.pth` 文件。
